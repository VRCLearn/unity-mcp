...
`manifest.json` must match. Stable VPM releases use the plain `<version>` value
from `package.json` as the tag and
are immutable.
...
After `main` is synchronized successfully, the workflow enumerates stable
CoplayDev GitHub Releases from the configured bootstrap version. Missing
`<version>` releases are processed in publication order. Each one constructs a
temporary snapshot from the exact upstream release tag and reapplies the
effective fork changes. This keeps VPM releases aligned with upstream releases
even when upstream `main` already contains newer, unreleased commits. Every
snapshot is packaged and tested before publication; its temporary branch is
removed afterward.
...
Each GitHub Release follows the official VRChat package template and attaches
the VPM ZIP, a UnityPackage with the same package ID and version, and the staged
`package.json`. The repository website is rendered from the pinned official
VRChat listing template.
